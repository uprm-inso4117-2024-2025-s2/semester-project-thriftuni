...
export default function ProductCard({id, title, price, img, latitude, longitude}: ProductCardProps) {
  ...
  useEffect(() => {
      async function getLocation() {
        ...
        setDistance(getDistance({latitude: location.coords.latitude, longitude: location.coords.longitude}, {latitude: parseFloat(latitude), longitude: parseFloat(longitude)}))
      }
    ...
    <Pressable style={styles.card} onPress={handleProductClick}>
      ...
      <Animated.View style={[styles.wishlist_button, { transform: [{ scale: scaleValue }] }]}>
        <Pressable hitSlop={{top: 7, bottom: 7, right: 7, left: 7}} onPress={handleWishlistClick}>
          <FontAwesome name={(isProductWishlisted) ? "heart" : "heart-o"} size={24} color={(isProductWishlisted) ? "red" : "gray"} />
        </Pressable>
      </Animated.View>
    </Pressable>
  )
